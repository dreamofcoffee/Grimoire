...
/**
 * The Reasonix chat projection, against a real `reasonix acp`.
 *
 * Nothing below the composition is a fake — see `chatProjectionLiveHarness`.
 *
 * Off by default. Run it with `GRIMOIRE_REASONIX_LIVE=1`; `reasonix setup` first.
 */
const live = process.env.GRIMOIRE_REASONIX_LIVE === '1' ? describe : describe.skip;

live('Reasonix chat projection live smoke', () => {
  jest.setTimeout(300_000);

  const CONVERSATION_ID = 'conv-reasonix-projection';
  ...
  function refuseVendorOutage(
    ...
    if (failure && /unavailable|service failure|upstream/i.test(failure)) {
      throw new Error(
        `Reasonix could not serve this row: ${failure} `
  ...
  function createPlugin(vault: string, overrides: Record<string, unknown> = {}): any {
    ...
    if (process.env.GRIMOIRE_REASONIX_MODEL) {
      settings.model = process.env.GRIMOIRE_REASONIX_MODEL;
    }
    return {
      settings,
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-reasonix-projection-'));
    ...
    const execution = new ReasonixExecution(createPlugin(vault, overrides), host.registry);
  ...
  it('row C: resumes the session after a reload, from what the vault kept', async () => {
    ...
    const shared = mkdtempSync(join(tmpdir(), 'grimoire-reasonix-projection-reload-'));
    ...
    // Reasonix's session, like every ACP one, belongs to the project it was opened
  ...
  // Reasonix reports usage over ACP inside the run it belongs to: `usage_update`
