...
import { ReasonixCommandCatalog } from '@/providers/reasonix/commands/ReasonixCommandCatalog';
...
describe('ReasonixCommandCatalog', () => {
  ...
  it('lists nothing before an active Reasonix session supplies commands', async () => {
    const catalog = new ReasonixCommandCatalog();

    await expect(catalog.listDropdownEntries({ includeBuiltIns: false })).resolves.toEqual([]);
    await expect(catalog.listVaultEntries()).resolves.toEqual([]);
    expect(catalog.defaultVaultStoragePath()).toBe('.reasonix/skills');
  });

  it('lists the vault skills from both roots the CLI scans, behind runtime duplicates', async () => {
    // `reasonix skills paths`: `.reasonix/skills` and the cross-provider `.agents/skills`.
    const catalog = new ReasonixCommandCatalog(adapter({
      '.reasonix/skills/guide/SKILL.md': '---\nname: guide\ndescription: A guide\n---\nUse this guide.',
