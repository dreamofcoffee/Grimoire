...
import { buildReasonixModelCatalogFingerprint } from '@/providers/reasonix/modelCatalogFingerprint';
...
describe('buildReasonixModelCatalogFingerprint', () => {
  ...
  it('changes when the CLI is upgraded in place', () => {
    ...
    const before = buildReasonixModelCatalogFingerprint(settings, '/usr/local/bin/reasonix', '');
    ...
    const after = buildReasonixModelCatalogFingerprint(settings, '/usr/local/bin/reasonix', '');

    expect(mockedFingerprint).toHaveBeenCalledWith('/usr/local/bin/reasonix');
  ...
  it('stays stable while the same build sits at the same path', () => {
    ...
    expect(buildReasonixModelCatalogFingerprint(settings, '/usr/local/bin/reasonix', ''))
      .toBe(buildReasonixModelCatalogFingerprint(settings, '/usr/local/bin/reasonix', ''));
  });
});
