...
/**
 * The three reference spaces one Reasonix turn passes through.
 *
 ...
 */
describe('ReasonixExecutionRequests', () => {
  const prompt: readonly AcpContentBlock[] = [{ type: 'text', text: 'Reply with exactly: ok' }];

  function createStore(overrides: Partial<ReasonixInvocationEnvironment> = {}): {
    store: ReasonixExecutionRequests;
    launches: number;
  } {
    let launches = 0;
    let next = 0;
    const store = new ReasonixExecutionRequests(
      () => `ref-${++next}`,
      async () => {
        launches += 1;
        return {
          ...
          cwd: '/vault',
          environment: { REASONIX_MODEL: 'opus' },
  ...
  it('resolves a turn into the launch and the prompt the backend dispatches', async () => {
    ...
    await expect(store.resolveLaunch(invocation.startupRef)).resolves.toEqual({
      executable: '/usr/local/bin/reasonix',
      // A subcommand: the recorded handshake is `reasonix acp`.
      arguments: ['acp'],
      cwd: '/vault',
      environment: { REASONIX_MODEL: 'opus' },
    });
  ...
  it('holds a prompt no longer than the dispatch that needed it', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Reasonix request reference.');
  });
  ...
  it('refuses a reference it never minted rather than guessing', async () => {
    ...
    await expect(store.resolve('ref-nonsense'))
      .rejects.toThrow('Unknown Reasonix request reference.');
    await expect(store.resolveLaunch('ref-nonsense'))
      .rejects.toThrow('Unknown Reasonix startup reference.');
    await expect(store.resolveDynamic('ref-nonsense'))
      .rejects.toThrow('Unknown Reasonix dynamic configuration reference.');
  });
  ...
  it('stays bounded when turns are composed and never dispatched', () => {
    ...
    return expect(store.resolve(first)).rejects.toThrow('Unknown Reasonix request reference.');
  });
  ...
  it('drops everything it is holding when the composition goes away', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Reasonix request reference.');
    await expect(store.resolveLaunch(startupRef)).rejects.toThrow('Unknown Reasonix startup reference.');
  });
});
