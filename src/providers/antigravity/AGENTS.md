...
- Plan indicators come from `AntigravityPlanUsageStore`, which invokes AGY's read-only `/usage` command and maps its Gemini and Claude/GPT 5-hour and weekly buckets onto the shared usage contract. Keep the poll non-interactive, do not read or persist OAuth tokens, and retain the last good snapshot when AGY is unavailable or logged out.
