...
import { ReasonixConversationHistoryService } from '@/providers/reasonix/history/ReasonixConversationHistoryService';
import type { ReasonixWorkspaceContext } from '@/providers/reasonix/ReasonixProviderModule';
 ...
 * not serve.
 */
export type ReasonixBoundConversation = () => BoundConversation | null;

export interface ReasonixModuleContextPorts {
  /**
 ...
 *
 * Only the history slots and the session commands are wired. Reasonix's workspace is
 * still registered the legacy way through `ReasonixWorkspaceServices`, and moving it
 ...
 */
export function createReasonixModuleContext(
  plugin: GrimoirePlugin,
  conversation: ReasonixBoundConversation,
  ports: ReasonixModuleContextPorts,
): ReasonixWorkspaceContext {
  const history = new ReasonixConversationHistoryService();
  const workspace = createWorkspaceContextSlots({
    chatUI: reasonixChatUIConfig,
    plugin,
    providerId: 'reasonix',
    services: () => maybeGetReasonixWorkspaceServices(plugin),
  });
  ...
  return {
    ...
    renderSettingsTab: host => {
      ...
      maybeGetReasonixWorkspaceServices(plugin)?.settingsTabRenderer
