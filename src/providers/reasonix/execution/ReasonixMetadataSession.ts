...
/** Where an isolated Reasonix process is launched, and what it is launched for. */
export interface ReasonixMetadataLaunch {
  readonly startupRef: string;
  readonly cwd: string;
  readonly mcpServers: AcpNewSessionRequest['mcpServers'];
}

export interface ReasonixMetadataSessionPorts {
  readonly clientFactory: ManagedAcpClientFactory;
  /** The process to spawn, prepared for this question and no conversation. */
  readonly launch: () => Promise<ReasonixMetadataLaunch>;
 ...
 * it. **Not `reasonix models list`**: that command lists what the account could
 ...
 */
export class ReasonixMetadataSession {
  constructor(private readonly ports: ReasonixMetadataSessionPorts) {}
  ...
  async discoverMetadata(): Promise<boolean> {
    ...
    try {
      ...
      const config = new ReasonixSessionConfigState({
