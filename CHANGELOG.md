...
- Reasonix as an opt-in provider over `reasonix acp`: models and modes from the live session, permission requests that carry their own subject, file writes approved by Grimoire, native resume, per-turn tokens read from Reasonix's own status notifications, and Grimoire-managed MCP servers in `.grimoire/mcp/reasonix.json`. Safe, Plan, and Auto-approve drive both of Reasonix's axes, because it keeps the session mode and the tool-approval posture apart. (#180)
